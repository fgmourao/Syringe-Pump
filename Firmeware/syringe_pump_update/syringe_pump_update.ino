...
void positioningMode()
{
  ...
  lcd.setCursor(0, 1);
  lcd.print(F("^ FWD   BWD v"));

  // Step delays define positioning speed
  // SHORTPRESS  -> fine movement
  // LONGPRESS   -> coarse / fast movement
  const uint16_t STEP_DELAY_FINE   = 900;
  const uint16_t STEP_DELAY_COARSE = 100;

  bool endstopHit   = false;  // Prevents repeated beeps at endstop
  bool motorEnabled = false;  // Enables driver only when needed

  // Ensure all buttons are released when entering the mode
  while (true)
  {
    getButtonState();
    if (buttonState == RELEASE) break;
  }
  ...
  while (true)
  {
    ...
    // ----- Exit positioning mode (LEFT button) -----
    if (buttonState == SHORTPRESS && lastButton == LEFT)
    {
      buttonState = MEDIUMPRESS;
      break;
    ...
    // ----- Reset position to zero (Right button) -----
    if (buttonState == SHORTPRESS && lastButton == RIGHT)
    {
      positioning_uSteps = 0;
      beep();
      buttonState = MEDIUMPRESS;
    }

    // ----- Periodic position update on LCD -----
    ...
    if (millis() > nextUpdate)
    {
      ...
      float mm = (float)positioning_uSteps /
                 (stepsPERmm * NOFMICROSTEPS);
    ...
    // Select speed based on press duration
    uint16_t stepDelay = STEP_DELAY_FINE;
    if (buttonState == LONGPRESS) stepDelay = STEP_DELAY_COARSE;

    // ===== FORWARD movement (UP button) =====
    if ((buttonState == SHORTPRESS || buttonState == LONGPRESS) &&
        lastButton == UP)
    {
      // Enable motor driver if not already enabled
      if (!motorEnabled)
      {
        digitalWrite(ENABLE_PIN, LOW);
        motorEnabled = true;
      ...
      // Check forward endstop
      if (digitalRead(ENDSTOP_PIN1) == LOW)
      {
        if (!endstopHit)
        {
          beep(40, 40);
      ...
      else
      {
        ...
        // Set motor direction
        if (INVERTDIRECTION) digitalWrite(DIRECTION_PIN, LOW);
        else digitalWrite(DIRECTION_PIN, HIGH);
    ...
    // ===== BACKWARD movement (DOWN button) =====
    else if ((buttonState == SHORTPRESS || buttonState == LONGPRESS) &&
             lastButton == DOWN)
    {
      // Enable motor driver if not already enabled
      if (!motorEnabled)
      ...
      // Check backward endstop
      if (digitalRead(ENDSTOP_PIN2) == LOW)
      {
        if (!endstopHit)
        {
          beep(40, 40);
      ...
      else
      {
        ...
        // Set motor direction
        if (INVERTDIRECTION) digitalWrite(DIRECTION_PIN, HIGH);
        else digitalWrite(DIRECTION_PIN, LOW);
    ...
    // ----- No button pressed: disable motor -----
    else
    {
      endstopHit = false;
  ...
  // Ensure motor is disabled before leaving the mode
  digitalWrite(ENABLE_PIN, HIGH);
